...
dc2 <- dc |> 
  mutate(cause_of_death = str_squish(str_remove_all(cause_of_death, "[[:punct:]]"))) |> 
  mutate(
    tuberculosis = str_detect(cause_of_death, "(?i)tubercul|phthisis|pthisis|consumption|terberculosis"),
    pneumonia = str_detect(cause_of_death, "(?i)pneumonia"), 
    typhoid = str_detect(cause_of_death, "(?i)typhoid"),
    asthma = str_detect(cause_of_death, "(?i)asthma"),
    meningitis = str_detect(cause_of_death, "(?i)meningitis"),
    peritonitis = str_detect(cause_of_death, "(?i)peritonit"),
    cardiac_issues = str_detect(cause_of_death, "(?i)cardiac|heart failure|heart trouble|mitral"),
    exhaustion = str_detect(cause_of_death, "(?i)exhaustion"),
    gastric_ulcer = str_detect(cause_of_death, "(?i)gastric.*ulcer|ulcer.*gastric"),
    encephalitis = str_detect(cause_of_death, "(?i)encephalitis"),
    malaria = str_detect(cause_of_death, "(?i)malaria"),
    asphyxia = str_detect(cause_of_death, "(?i)immediate: asphyxia"),
    asthenia = str_detect(cause_of_death, "(?i)asthenia"),
    lung_disease = str_detect(cause_of_death, "(?i)lungs|oedema of lungs|rage of lungs"),
    hemorrhage = str_detect(cause_of_death, "(?i)hem|rhage|hemm|y hemm|wound of groin by hog, hem"),
    heart_failure = str_detect(cause_of_death, "(?i)heart failure"),
    influenza = str_detect(cause_of_death, "(?i)influenza"),
    no_details = is.na(cause_of_death) | str_detect(cause_of_death, "(?i)NA|missing|unknown|information")
  )
```
...
summary_long |> 
 ...
 ggplot(aes(x = death_year, y = count, fill = count)) +
  ...
  labs(title = "Tuberculosis Deaths at Cheltenham, Jessup",
       ...
       caption = "Graphic by Rob Wells, 11-30-2025",
       y="Number of Deaths",
       x="Year")
