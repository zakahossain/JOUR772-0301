...
**A1.** There are 32 departments offering a course with "climate" in its title and has at least 1 seat. 'Atmospheric and Oceanic Science' has offered the most.

```{r}
courses_on_climate_1_seat <- umd_courses |>
...
**A2.** Finance. Did not think folks at finance cared about the environment.
...
dept_with_syllabus <- umd_courses |>
...
# Q from pre_lab: What percent of funds came from an institution v. from an individual?
**A6.**

```{r}
with_contributor_type <- maryland_expenses |>
  mutate(
    contributor_type = case_when(
      str_detect(payee_type, "Candidate Committee") ~ "Non_individual",
       str_detect(payee_type, "Self (Candidate)")  ~ "Individual",
      .default = "Non-Individual"
    )
  )
...
```{r}
by_contri_type <- maryland_expenses |>
  mutate(
    contributor_type = case_when(
      str_detect(payee_type, "Individual") ~ "Individual",
       str_detect(payee_type, "Self (Candidate)")  ~ "Self (Candidate)",
      .default = "Non-Individual"
    )
  ) |>
