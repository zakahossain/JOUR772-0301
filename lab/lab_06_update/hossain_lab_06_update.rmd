...
```{r}
calls_by_county_month <- combined_911 |>
...
Yes, there is a pattern -- all three counties show a similar seasonal pattern with increased overdose calls during May - August, peaking in August. Cecil County had the highest single month with 55 calls in August. The most significant outlier is Allegany County's November w/ just 17 calls, almost 50% its typical monthly average. It also had a notably low January w/ 18 calls. 

Q2b: Use "combined_911" data and write code to calculate the most calls per county and location. 

```{r}
calls_by_county_location <- combined_911 |>
  group_by(county, location) |>
  summarise(total_calls = n(), .groups = 'drop') |>
  arrange(desc(total_calls))

head(calls_by_county_location, 10)
```
...
314 GROVE NECK RD EARLEVILLE, MD (Cecil County) had the most overdose calls in the combined three-county dataset. It is a drug and alcohol rehabilitation facility named 'Bracebridge Hal.' That makes sense.
...
# (already did this earlier, but doing it again here)
maryland_zcta <- read_csv('data/maryland_zcta.csv') |>
...
#read up on left and right joins, i now understand how they work!

```{r}
high_youth_zips <- baltimore_911_with_demographics |>
  mutate(pct_under_18 = (PLT18SP / POP100) * 100) |>
  group_by(zip_code, POP100) |>
  summarise(
    total_calls = n(),
    pct_under_18 = first(pct_under_18),
    .groups = 'drop'
  ) |>
  rename(population = POP100) |>
...
There is no zip codes where the percentage of under 18 population is at least 75%
...
Zip code 21217 has the highest rate with 20.86 calls per 1,000 people. Neighborhoods include: Druid Heights, Central Baltimore --  these are historically working-class West Baltimore neighborhoods that are now predominantly African American with significant economic challenges. Hardships --> drug overdose.
