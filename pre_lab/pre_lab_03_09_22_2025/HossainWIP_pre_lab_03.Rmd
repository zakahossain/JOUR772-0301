...
**Answer** It says there is a parsing issue and asks me to run 'problems(payments)' for details. I get 369 rows of problems.
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It does not show a parsing error, although it tells me to "Use `spec()` to retrieve the full column specification for this data."
...
**Answer** There are 553 zip codes that are less than 5 characters in length. These zip codes possibly start with a 0 (zero) and therefore the preceding zero is dropped.
