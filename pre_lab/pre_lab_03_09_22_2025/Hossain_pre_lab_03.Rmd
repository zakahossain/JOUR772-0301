...
**Answer** It says there is a parsing issue and asks me to run 'problems(payments)' for details. I get 369 rows of problems. And it is in spaces it expects a <dbl> but there is <chr> in that cell
...
**Answer** It does not show a parsing error, although it tells me to "Use `spec()` to retrieve the full column specification for this data." Also vendor zip and date are somehow <chr> type.
...
**Answer** The "Vendor Name" field is <chr> type. The "Amount" field is <dbl> type. The "Date" column is <chr> type.
...
**Answer** There are 553 zip codes that are less than 5 characters in length. These zip codes possibly lead with a 0 (zero) and therefore the preceding zero is dropped. Could be other reasons.
...
**Answer** The N/A fields are problematic because of missing data and there's 50 entries in 2013 in 'L' category, possibly loan but not certain. There's a third catagory which is contract -- a category not mentioned in the question.
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** The grantees are not catagorised as individuals or organisations which may be potentially problematic. Also, there is very few small ticket grants.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** The first letter capitalisation has been taken out and underscore has replaced space -- all the things that coding languages like.
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** Line 186 is renaming grantor as 'source', next line is making the three mentioned columns upper-case -- source, grantee and description.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** There are 58 duplicate entries with a duplicate count of 2 each (which may mean 29 entries have been made twice!?)
#### Question: Are there 58 entries that have duplicates or 58 entries in total? I also see repetition of more than 2x on some of these, like rows 17-20 are the same.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** New dataframe has 19,453 rows, 29 less rows from 19,482.

#### Which means my previous answer correctly inferred that 29 entries have been made twice, still does not answer why rows 17-20 are the same, how is that entry duplicated twice for a total of 4 entries?)
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** A new column zip5 is created with 5 digit zip codes on it. The actual zip_code column remains untouched with some zip codes longer than 5 characters, some separated by a hyphen, some with hyphen and spaces on both sides and some written in conjunction without any breaks. 
#### note: row 977 has a non zip-code in the zip-code column.
